...
# Configuration
REPO_ID = "lerobot/unthread3" if UNTHREAD else "lerobot/thread3"
DATASET_NAME = "unthread3" if UNTHREAD else "thread3"
ORIG_DATASET_PATH = Path("/home/jeremiah/openteach/extracted_data/thread3/h5_files")
FPS = 20
...
FEATURES = {
    ...
    "observation.state": {
        "dtype": "float32",
        "shape": (15,),
        "names": ["joint1", "joint2", "joint3", "joint4", "joint5", "joint6", "joint7", "gripper_pos", "joint1_tau", "joint2_tau", "joint3_tau", "joint4_tau", "joint5_tau", "joint6_tau", "joint7_tau"],
    },
...
def main():
    ...
    if UNTHREAD:
        h5_files = sorted(f for f in ORIG_DATASET_PATH.glob("*.h5") if "unthread" in f.name)
    else:
        h5_files = sorted(f for f in ORIG_DATASET_PATH.glob("*.h5") if not "unthread" in f.name)
